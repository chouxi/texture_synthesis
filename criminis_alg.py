...
class criminis_algorithm:
    ...
    def remove_blocks(self, block_list):
        ...
        io.show()
        tmp_rgb = np.zeros((sample_x + margin * 2, sample_y+ margin*2,3))
        tmp_rgb[margin:sample_x + margin, margin: sample_y + margin,] = self.image_rgb
        self.image_rgb = tmp_rgb
    ...
    def do_criminis(self):
        ...
        self.image = self.image[margin: self.image.shape[0] - margin, margin: self.image.shape[1] - margin] *255
        self.image_rgb = self.image_rgb[margin: self.image.shape[0] - margin, margin: self.image.shape[1] - margin,]
        io.imshow(self.image_rgb)
        io.show()
